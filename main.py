...
if consejo == "Consejo 1. Dunia Guzman":
    ...
    if encuestado == "Dunia Guzman":
        ...
        st.divider()
        relevante = st.radio("Selecciona al más relevante", consejoR, index=None, horizontal=True)
        st.divider()
        if relevante == "Gabriela Sánchez":
            consejoNR.remove(relevante)
            menos_relevante = st.radio("Selecciona al menos relevante", consejoNR, index=None, horizontal=True)
